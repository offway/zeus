...
public interface PhGoodsRepository extends JpaRepository<PhGoods,Long>,JpaSpecificationExecutor<PhGoods> {
	...
	@Query(nativeQuery=true,value="select g.* from ph_goods g ,ph_brand b where g.brand_id = b.id and b.id=?1  and g.`label`='0' and g.`status`='1' and b.`status`='1' order by g.sale_count desc,g.up_time desc limit 10")
