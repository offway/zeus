...
public interface PhProductInfoRepository extends JpaRepository<PhProductInfo,Long>,JpaSpecificationExecutor<PhProductInfo> {
	...
	@Query(nativeQuery=true,value="select * from ph_product_info where  channel& ?1=?1 and status='1' and  NOW() >= begin_time and NOW() < end_time order by sort desc,begin_time desc")
	...
	@Query(nativeQuery=true,value="select * from ph_product_info where channel& ?1=?1 and status='1' and NOW() >= end_time  order by sort desc,end_time desc")
	List<PhProductInfo> findByBefore(int channel);
	
	@Query(nativeQuery=true,value="select count(*) from ph_product_info where  channel& ?1=?1 and status='1' and  NOW() >= begin_time and NOW() < end_time")
	...
	@Query(nativeQuery=true,value="select count(*) from ph_product_info where channel& ?1=?1 and status='1' and NOW() >= end_time")
	int countByBefore(int channel);
	
	@Query(nativeQuery=true,value="select * from ph_product_info where  channel& ?1=?1 and status='1' and  NOW() >= begin_time and NOW() < end_time order by sort desc,begin_time desc limit ?2,?3")
	...
	@Query(nativeQuery=true,value="select * from ph_product_info where channel& ?1=?1 and status='1' and NOW() >= end_time order by sort desc,end_time desc limit ?2,?3")
	...
	@Query(value="select new cn.offway.zeus.dto.ProductJoin(p.id,p.image,p.name,t.createTime,p.beginTime,p.endTime,p.shareImage,p.shareTitle,p.shareDesc) from PhLotteryTicket t,PhProductInfo p where p.id=t.productId and t.unionid=?1 and t.source='0' and bitand(p.channel,?2)=?2 order by p.endTime desc")
